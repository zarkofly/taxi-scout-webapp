...
function App() {
  ...
  const [isLadyTaxi, setIsLadyTaxi] = useState(() => {
    const saved = localStorage.getItem("isLadyTaxi");
    return saved ? JSON.parse(saved) : false;
  });

  // Čuvaj isLadyTaxi u localStorage
  useEffect(() => {
    localStorage.setItem("isLadyTaxi", JSON.stringify(isLadyTaxi));
  }, [isLadyTaxi]);

  // Helper funkcija za dohvatanje kolačića
      ...
      <LadyTaxiContext.Provider value={{ isLadyTaxi, setIsLadyTaxi }}>
        <UserContext.Provider value={{ userType, setUserType }}>
          <SourceContext.Provider value={{ source, setSource }}>
            ...
            <div
                className={`App ${isLadyTaxi ? "lady-taxi-theme" : ""}`}
                style={
                  isLadyTaxi
                    ? { backgroundColor: "#FF69B4", minHeight: "100vh" } // Pink background for ladyTaxi
                    : {
